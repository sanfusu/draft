...
members and friends: // % BT
一个类的 friend 是一个允许使用类中的私有和保护成员名的函数或者类。
一个类通过友元声明指示其友元。
\uwave{这种声明向其友元给出特殊的权限，但是并不会将友元作为被援助类的成员：}
...
A class shall not be defined in a friend declaration. \\  % BT
一个类不因当作为一个友元声明定义 % ET
\begin{example} 
...
在友元声明中第一个声明的函数拥有其作为成员的命名空间的链接\iref{basic.link}。
否则，该函数保留其原有的链接\iref{dcl.stc}。
...
a class
\tcode{Y}.
\indextext{member function!friend} \\ % BT
当一个友元声明指示一个重载名或运算符，只有通过参数类型指定的函数是一个友元。类 \tcode{X} 的成员函数
可以是类 \tcode{Y} 的一个友元。\indextext{member function!friend} % ET
\begin{example}
