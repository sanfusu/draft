...
除了 \tcode{thread_local} 可能会和 \tcode{static} 或者 \tcode{extern} 一起出现之外
在给定的 \grammarterm{decl-specifier-seq} 中至多有一个 \grammarterm{storage-class-specifier}。如果 \tcode{thread_local}
出现在任意一个变量声明中，则应当出现该条目的所有声明中。如果一个 \grammarterm{storage-class-specifier}
出现在一个 \grammarterm{decl-specifier-seq} 中，在相同的 \grammarterm{decl-specifier-seq} 和
\grammarterm{init-declarator-list} 不能有 \tcode{typedef} 说明符，
或声明中的 \grammarterm{member-declarator-list} 不应为空（除了在一个具名的命名空间或全局命名空间中声明的一个匿名联合，
其应当被声明为 \indextext{specifier!\idxcode{static}}%
\tcode{static}\iref{class.union.anon}）。\grammarterm{storage-class-specifier} 
通过列表中的每一个 \grammarterm{storage-class-specifier} 应用到声明的名字。一个除 \tcode{thread_local} 之外的
\grammarterm{storage-class-specifier} 不因当在显式特殊化\iref{temp.expl.spec} 或一个显示实例化 \iref{temp.explicit} 指令中指定。
...
\end{note} \\
\begin{note}
在块范围内不使用 \grammarterm{storage-class-specifier} 声明的变量
或者被声明为函数参数时具有默认的自动存储期 \iref{basic.stc.auto}。
\end{note}
...
\grammarterm{decl-specifier-seq}. \\
\tcode{thread_local} 说明符表示其具名条目具有线程存储期\iref{basic.stc.thread}。
只能应用到命名空间中的变量名或块范围内应用到静态数据成员名。
当 \tcode{thread_local} 被应用到块范围内的变量时，
\grammarterm{decl-specifier-seq} 中若无其它 
\grammarterm{storage-class-specifier} 出现时，
\grammarterm{storage-class-specifier} 
\tcode{static} 将被使用。
...
\grammarterm{Function-specifier}{s} 只能被用在函数声明中。
...
\tcode{virtual} 说明符只可以被用在非静态类成员函数中的初始声明；
详情请看~\ref{class.virtual}。
...
\tcode{explicit} 说明符只可以用在类定义中的一个构造函数或转换函数；
详情请看~\ref{class.conv.ctor}和~\ref{class.conv.fct}。
...
\tcode{friend} 说明符被用来说明对类成员的访问；详情请看~\ref{class.friend}。
...
\tcode{decltype(}\grammartermnc{expression}\tcode{)}
...
in an element of the inline namespace set\iref{namespace.def}
...
for the derived class\iref{class.copy},
...
an assembler.
\end{note} \\
\tcode{asm} 声明是条件支持的；也就是说是 \impldef{meaning of \tcode{asm} declaration}。
\grammarterm{asm-definition} 中可选的 \grammarterm{attribute-specifier-seq} 和 \tcode{asm} 声明有关。
\begin{note}
一般用来将信息通过具体实现传递给汇编器。
\end{note}
