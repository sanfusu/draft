...
一个名字可以通过在嵌套的声明区域中或者派生类\iref{class.member.lookup} 
中显式声明相同的名字来隐藏。
...
类名\iref{class.name} 或枚举名\iref{dcl.enum} 可以被相同域中声明的变量、数据成员、函数或枚举器的名字
隐藏。如果一个类或枚举名和一个变量、数据成员、函数、或枚举器以相同名字被声明在同一个域中（以任何顺序），
只要变量、数据成员、函数或枚举器名字是可见的，类或枚举名都会被隐藏。
...
在一个成员函数定义中，块域中声明的名字会隐藏具有相同名字的类成员的声明；
详情请看~\ref{basic.scope.class}。派生类\iref{class.derived}中一个成员的声明会隐藏
一个基类中具有相同名字的成员声明；详情请看~\ref{class.member.lookup}。
...
在查询一个被命名空间名限定的名字时，通过一个 \grammarterm{using-directive} 可见的声明
可以被包含该 \grammarterm{using-directive} 的命名空间中的相同名隐藏；详情请看~\ref{namespace.qual}。

\pnum
\indextext{visibility}%
If a name is in scope and is not hidden it is said to be \defn{visible}. \\ %
如果一个名字在范围之内并且没有被隐藏，则称为 \defn{visible}
\indextext{scope|)}
