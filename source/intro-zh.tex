...
对于类和类模板，库部分列举部分定义。私有成员\iref{class.access} 虽未被列举，
...
库中定义的名字具有命名空间范围\iref{basic.namespace}。
...
申请函数申请内存失败时，\grammarterm{new-initializer} 中表达式的计算\iref{expr.new}）。
...
这些被统称为一个程序的 \defnx{observable behavior}{behavior!observable}。
...
expression as \\
由于这些运算符具有关联性和优先级，因此 \tcode{(a+32760)} 的和会被加到 \tcode{b} 上，
相应的结果随后被加上 5 ，最后结果被赋予给 \tcode{a}。
在一个溢出会提供异常的机器上以及 \tcode{int} 的可表示范围为 \crange{-32768}{+32767}，
则具体实现无法将该表达式重写为：
\begin{codeblock}
a = ((a + b) + 32765);
\end{codeblock}
...
either as \\
这是因为 \tcode{a} 和 \tcode{b} 的值相应为 -32754 和 -15，\tcode{a + b} 的和将提供异常，而原表达式不会；
表达式也不能重写为
\begin{codeblock}
a = ((a + 32765) + b);
\end{codeblock}
...
这是因为 \tcode{a} 和 \tcode{b} 的值已经相应的变为 4 和 -8 或者 -17 和 12。
...
（\defn{library clauses} 描述了 \Cpp{} 标准库。
...
贯穿整个文档，每一个以 ``\noteintro{Example}'' 开头，并以 ``\noteoutro{note}'' 结尾都会引入一个注解。
示例和注解可能会嵌套。
\indextext{standard!structure of|)}
...
``\opt{\relax}'', so \\
本文档中使用的记号格式，句法类别使用 \grammarterm{italic} 类型指示。
...
\grammarterm{X-name} 是由上下文环境决定其意义的标识符（比如，\grammarterm{class-name}, 
\grammarterm{typedef-name}）。
\item \grammarterm{X-id} is an identifier with no context-dependent meaning
(e.g., \grammarterm{qualified-id}). \\
\grammarterm{X-id} 是不由上下文决定其意义的标识符（比如, \grammarterm{qualified-id}）。
...
declarations). \\
\grammarterm{X-seq} 是一个或多个不被分隔符干扰的 \grammarterm{X}
（比如，\grammarterm{declaration-seq} 是一个声明序列）。
...
\grammarterm{X-list} 是一个或多个被间插逗号分离的 \grammarterm{X}
（比如，\grammarterm{identifier-list} 是一个被逗号分离的标识符序列）
\end{itemize}%
\indextext{notation!syntax|)}
